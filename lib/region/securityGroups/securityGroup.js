// Generated by CoffeeScript 1.4.0
(function() {
  var BaseModel, gumbo, outcome,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

  gumbo = require("gumbo");

  BaseModel = require("../base/model");

  outcome = require("outcome");

  module.exports = (function(_super) {

    __extends(_Class, _super);

    function _Class() {
      return _Class.__super__.constructor.apply(this, arguments);
    }

    /*
        {
          ports: [
            {
              from: 80,
              to: 8080,
              type: "tcp",
              ranges: ["0.0.0.0/0"]
            }
          ]
        }
    */


    _Class.prototype.authorizePorts = function(optionsOrPort, callback) {
      var i, j, n, options, portInfo, query, range, _i, _j, _len, _len1, _ref, _ref1;
      if (typeof optionsOrPort === "number") {
        options = {
          ports: [
            {
              from: optionsOrPort,
              to: optionsOrPort
            }
          ]
        };
      } else {
        options = optionsOrPort;
      }
      query = {
        GroupId: this.get("_id")
      };
      _ref = options.ports;
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        portInfo = _ref[i];
        n = i + 1;
        if (!portInfo.ranges) {
          portInfo.ranges = ["0.0.0.0/0"];
        }
        query["IpPermissions." + n + ".IpProtocol"] = portInfo.protocol || "tcp";
        query["IpPermissions." + n + ".FromPort"] = portInfo.from || portInfo.number;
        query["IpPermissions." + n + ".ToPort"] = portInfo.to || portInfo.number;
        _ref1 = portInfo.ranges;
        for (j = _j = 0, _len1 = _ref1.length; _j < _len1; j = ++_j) {
          range = _ref1[j];
          query["IpPermissions." + n + ".IpRanges." + (j + 1) + ".CidrIp"] = range;
        }
      }
      return this._ec2.call("AuthorizeSecurityGroupIngress", query, outcome.e(callback).s(function(result) {
        return callback(null, result);
      }));
    };

    /*
       destroys the keypair
    */


    _Class.prototype.destroy = function(callback) {
      var _this = this;
      return this._ec2.call("DeleteSecurityGroup", {
        GroupName: this.get("name")
      }, outcome.e(callback).s(function() {
        return _this.collection.load(callback);
      }));
    };

    return _Class;

  })(BaseModel);

}).call(this);
